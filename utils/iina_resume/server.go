...
func newApplication(runner commandRunner, logger *slog.Logger) *application {
	return &application{iina: newIINAService(runner), logger: logger}
}
...
// handlePlayback reports absence as a valid empty state rather than an HTTP
// failure; the page can then explain how to create the first IINA record.
func (app *application) handlePlayback(writer http.ResponseWriter, request *http.Request) {
	...
	record, err := app.iina.LastPlayback(ctx)
	if errors.Is(err, errNoPlaybackHistory) {
		writeJSON(writer, http.StatusOK, map[string]any{"playback": nil})
		return
	}
	if err != nil {
	...
	writeJSON(writer, http.StatusOK, map[string]any{"playback": record})
}

// handleResume accepts no caller-supplied path. The backend always re-reads
// IINA's own record, eliminating arbitrary-command and arbitrary-file inputs.
func (app *application) handleResume(writer http.ResponseWriter, request *http.Request) {
	if !sameOrigin(request) || !strings.HasPrefix(request.Header.Get("Content-Type"), jsonContentType) {
		writeJSON(writer, http.StatusForbidden, map[string]string{"error": "请求来源无效"})
		return
	}
	ctx, cancel := context.WithTimeout(request.Context(), apiTimeout)
	defer cancel()
	record, err := app.iina.Resume(ctx)
	if err != nil {
		status := http.StatusInternalServerError
		if errors.Is(err, errNoPlaybackHistory) || errors.Is(err, errSourceUnavailable) {
