...
因此，这个工具的核心不是“找到一个 URL”，而是建立一条连续的信任转换链：把不可信文本收敛为受限制的分享地址，把易变化的页面结构压缩为稳定的 `WorkInfo`，再把一个视频或一组有序图片转换成受控的文件、目录或浏览器字节流。每一层只接受上一层已经验证过的状态，并在跨越网络、文件系统和浏览器边界时重新校验关键不变量。
...
`WorkInfo` 不是页面数据的镜像，而是页面解析层与交付层之间的稳定契约。`Kind` 只能是视频或图集；视频必须恰好包含一个视频资产，图集必须包含至少一个图片资产。每个 `MediaAsset` 保存 URL、媒体类型、扩展名和可选尺寸。`Validate` 在解析边界冻结这些不变量，下载层不需要知道 `_ROUTER_DATA`、路由键或水印字段。
...
flowchart LR
    ...
    H --> I[逐个受信任媒体响应]
    I --> J{交付入口}
    J -->|CLI 视频| K[临时文件与原子发布]
    J -->|CLI 图集| M[独占目录与失败回滚]
    J -->|Web| L[短期单媒体票据与逐项代理]
```
...
模块边界还带来一个测试上的直接收益。`webApplication` 依赖的是 `workResolver` 和 `mediaOpener` 两个窄接口，测试可以用本地假对象替换外部网络，却仍然覆盖真实的 HTTP 路由、票据状态、MP4 响应、图片预览和独立下载。这种接口不是为“抽象”而抽象，它服务的是唯一明确的变化点：外部依赖必须能够从进程内测试中移除。
...
聊天工具会把某些链接复制为 Markdown，例如 `[https://...\\_...](https://...\\_...)`；内容经过多次转发或序列化后，同一个下划线前还可能累积多个连续反斜杠。若 URL 候选不在 `]` 和 `(` 处停止，程序会把两个地址及 Markdown 包装吞成一个路径；若只去掉一层 `\\_`，剩余反斜杠仍会改变短链 token。两种情况都可能让抖音返回与真实风控页相同的校验正文。输入层因此先隔离单个候选，再把下划线前任意正数个连续反斜杠归一化。这个错误必须在分享层消化，不能让 resolver 猜测错误地址的含义。
...
图文作品的关键差异不是接口地址，而是详情响应的数据分支。真实用例仍可通过固定的 `/video/{itemId}` 兼容页面触发相同详情接口，但 `aweme_detail.images` 包含有序图片数组，同时 `video` 字段可能只承载背景音乐。解析器因此先判断 `images`，再考虑视频，避免把音乐 MP3 误判为作品视频。每张图片只读取 `url_list`；现场响应中的 `download_url_list` 使用 `dy-water-v2` 变换，属于带水印版本，不能因为字段名含有“download”就优先选择。

同一张图通常提供 WEBP 和 JPEG 等多个公开展示地址。程序在受信任的 `douyinpic.com` 主机内优先选择 JPEG，以兼容更多本地查看器；若没有 JPEG，再接受 WEBP 或 PNG。公开详情与 CDN 会动态协商规格：同一真实用例在同一天先返回 1440×2560，最终逐图验收又返回 2160×3840。程序不会改写签名 URL 冒充更高规格，只保存当次公开响应。这里的“原图”指公开详情提供的最高可用无水印展示图，不声明它与创作者上传文件逐字节一致。
...
resolver 产出的每个 `MediaAsset.URL` 在真正请求前还会被 downloader 再验证一次。这个重复不是浪费：`WorkInfo` 可能来自测试替身，未来也可能来自缓存或其他 resolver。把验证放在出站请求的最后边界，可以保证任何调用方都无法绕过媒体域名策略。媒体网关跳转到 CDN 时，每一跳仍会重新校验。
...
图集在 CLI 中采用“一个作品一个目录”的交付契约。程序先通过 `mkdir` 原子地取得独占目录名，再按作品顺序写入 `01.jpg`、`02.jpg`。每张图片仍经过临时文件和原子发布；若中途任何一张失败，程序只删除本次独占目录，从而既不留下半套图集，也不会误删用户已有内容。同名目录同样使用数字后缀解决冲突。
...
服务端完成完整解析后，为每个可下载媒体签发一个短期票据。票据由 24 个密码学随机字节编码而成，有效期十分钟；它保存作品和媒体索引，不携带上游媒体地址。视频只返回一个 `GET /api/download/{ticket}`。图集为每张图片返回 `GET /api/preview/{ticket}` 与 `GET /api/download/{ticket}`，前者不设置附件响应头，可直接放入同源 `<img>`；后者设置带作品信息和图片序号的文件名。每次请求都重新验证并打开对应 CDN 地址，一张图片失败不会阻止用户查看或下载其他图片。
...
下载接口使用不可猜测的短期票据，而不是接受一个任意 `url` 参数。这一设计同时完成了三件事：浏览器看不到上游网关，下载动作只能引用已经解析成功的 `WorkInfo`，服务端还会在每个实际出站请求前再次执行媒体安全策略。它本质上是把一个强能力，也就是“让本机服务访问一组媒体地址”，缩小为一个有时效、有来源约束的局部能力。
...
自动化测试使用 `httptest.Server` 和窄接口替身，覆盖完整 Markdown 分享文本、仿冒域名、短链重定向、风控页最终 URL 恢复、移动端请求头、旧 SSR 数据解析、新 SSR `itemId` 调度、浏览器详情作品绑定、最高 H.264 选择、真实图文结构、图片顺序、水印字段排除、图集失败回滚、逐图预览和下载、票据过期及附件响应。完整 Web 测试还会确认解析响应没有泄漏媒体 URL，再分别请求假视频或假图片票据，从而贯穿“解析、授权、下载”整个本地协议。
...
如果继续演进，合理顺序不是先增加更多解析技巧，而是沿现有边界扩展能力。下载进度可以在流式复制处增加计数字节；断点续传需要让 `OpenAsset` 支持并验证 Range 语义，并重新设计临时文件恢复；多解析器可以实现同一个 `workResolver` 接口；多人部署则必须替换进程内票据、增加认证、配额、审计和完整可观测性。每一步都应明确自己改变了哪条不变量，而不是把新分支塞进当前主流程。
