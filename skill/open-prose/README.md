...
若希望在本机 Open Claw 中使用此插件（含飞书 stage markdown 转图下发），可安装到 `~/.openclaw/extensions`：
...
- **Stage markdown → image delivery**：在飞书或 QQ 渠道运行 investment 类 `.prose` 时，各 stage 的 markdown 会渲染成图片并发送到当前会话（飞书需在 `openclaw.json` 中配置 `channels.feishu` 且当前会话的 `ctx.to` 为 chat_id）
...
发完后会跑多阶段投资研究流程，并在飞书里把各 stage 的 markdown 转成图片发出（若已配置飞书）。
...
2. **多阶段 + 转图发飞书** 只在**插件路径**里实现：  
   - 插件里是 `executeInvestmentWorkflow`（11 个 stage：planner / financial / industry / news / macro / sentiment / thesis / risk / critic / report），每个 stage 会写 `stages/*.md`，最后用 `renderMarkdownToImage` + 飞书 API 发图。  
   - VM 路径（invest_muyuan.prose）是 agent 用 Task 起 subagent，没有走插件的「stage → 图片 → 飞书」逻辑，所以**不会有图片**。
...
- **目标**：让 **插件的 `/prose` 命令** 处理 `/prose invest ...`，这样才会跑 11 阶段管线并发图。
- **当前**：在飞书里发 `/prose invest ...` 时，OpenClaw 把整句当普通消息给了 agent，所以走的是 VM + invest_muyuan.prose，不是插件。
- **建议**：
  1. **先用 CLI 验证插件路径**（能跑满 11 阶段且会写 stages、可本地看是否发图）：  
     ```bash
  ...
  3. **临时绕开**：在飞书里不发 `/prose invest`，而是发一句明确「用插件跑投资研究」的指令（若 agent 会调用插件或能触发插件命令的话）；或者继续用 CLI 跑 `/prose invest` 拿完整报告和图片。

总结：**不是 OpenProse 多 agent 或转图逻辑坏了，而是飞书这条消息没进插件，只进了 VM 路径，且 VM 只跑了一个 session 就结束了。** 要多阶段 + 图片，需要让 `/prose invest` 走插件命令路径（例如通过 CLI 或调整 Feishu 命令路由）。
