---
name: tdd-ha-bridge
description: 为普通开发者生成完整、清楚、可独立审查的测试或 AI agent eval 设计与运行报告；用业务语言讲清检查范围、六类 Lens 覆盖、具体场景、预期与实际表现、未证明风险和待决策事项，并在内部用风险建模、预期来源、验证真实度与错误实现挑战保证严谨性。适用于“先审场景和验证方法、再写测试或实现”以及实际 eval 结果解释；不用于机械追求用例数或覆盖率。
---

# TDD Human-Approval Bridge

这个 skill 交付的不是“尽可能多的测试用例”，而是一份普通后端开发者可以独立读懂、核查和审批的测试或 eval 文档。篇幅由问题本身决定，不设阅读时长、字数、场景数或待决策问题数目标。文档必须完整说明：
...
6. **Internal rigor, external clarity**：内部保留完整证据链；主文档用业务语言呈现“测了什么、为什么这样测、发生了什么、哪里没测到、需要决定什么”，并把支撑重要结论的场景、预期和实际表现讲完整。摘要不能代替证据，篇幅也不用于衡量可读性。

## 默认读者与两类产物

默认读者是熟悉业务和代码、但不要求掌握测试理论术语的普通后端开发者。主文档是本次设计或评估的完整事实来源；读者不应为了理解任何重要结论而被迫打开附件。

### 开发者审批主文档

必须让读者不追踪任何 `B/R/T/V/M` ID，就能回答：

1. 这次验证什么、使用了哪些输入或环境、是否已经实际运行？
2. 六类 Lens 中哪些已覆盖、部分覆盖、未覆盖或与本次无关？
3. 每个相关 Lens 具体检查了哪些现实风险，状态判断由哪些场景支撑，还缺什么？
4. 每个重要场景的背景、触发过程、预期结果、验证办法和当前验证状态是什么？
5. 已经实际运行时，模型或实现真正做了什么；存在对照时双方分别怎样表现；为什么据此通过、失败或无法确定？
6. 目前哪些测试已经跑通，哪些只是纸面场景，哪些完全没有验证手段？
7. 哪些问题需要人做业务选择？推荐选项及影响是什么？
8. 批准后下一步是什么？
...
内部分析始终执行。用户要求复现、存在高风险未决项需要精确追踪，或产物需要正式审计时，可以另写 `*.evidence.md`。附件保存 Source Ledger、`B/R/T/V/M` 全量映射、完整原始轨迹、哈希、脚本、机器评分、验证真实度明细和错误实现清单等复现材料。

附件不能承载理解结论所必需的解释。场景背景、预期结果、实际表现、判断理由、用户影响和重要限制都属于主文档。若附件显示某项只是纸面覆盖，主文档必须明确写“部分覆盖”或“未覆盖”，不能只在附件降级。
...
- 不要用证据的存放形式代替证据内容。“检查了实际轨迹”“做了逐条评分”“发现一次确认缺口”都没有说明发生了什么，不能单独作为结论。至少把代表性场景、预期行为、实际行为和影响写进正文。
- 结果表只用于让读者定位结论，不能承担全部解释。每个会改变总体判断的失败、部分通过、条件差异或评分分歧，表后必须有一个可独立读懂的完整证据故事。
- 不要惜字如金。应删除的是重复映射、无关方法论和没有决策价值的细节，不是帮助读者理解因果的上下文、先后顺序和双方表现。
- 文档较长时使用目录、清楚的标题和按业务场景分组来帮助阅读；不得通过删除必要解释、使用缩写或把内容移到附件来缩短正文。
- 表格是索引和比较工具。一个单元格容纳不下完整因果过程时，在表后展开正文；不要用表格列数反向限制需要说明的信息。
...
- `P2`：防御性保障；
- `P3`：极端或低价值；不会改变审批、实现选择或当前风险判断时可以不展开，并在范围说明中交代取舍。

风险必须具体到失效机制，例如“DB commit 成功、publish 失败后重试造成重复业务效果”，不要写“异常情况”。

### 6. 选择高价值场景

场景不是分类清单。优先选择同时验证行为义务、暴露高风险故障、覆盖关键边界的代表性 witness。所有会改变批准结论、实现选择或主要风险判断的场景都要进入主文档；只换无关样例值的重复变体可以合并。只在交互风险真实存在时做 pairwise/t-way 组合，不展开笛卡尔积。
...
严格使用 [references/output-contract.md](references/output-contract.md) 的内容契约。主文档完整说明：

1. 当前需求、评估对象、范围、输入与是否实际运行；
2. 当前结论，以及哪些证据支持这个结论；
3. 六类 Lens 分别检查了什么、覆盖到哪里、由哪些场景支撑、还缺什么；
4. 用业务故事组织的具体场景，包括选择理由、前置状态、触发过程、预期、验证办法和当前状态；
5. 已运行时的实际表现、对照差异、判断理由和实际影响；
6. 所有会改变预期或实现的待决策事项；
7. 尚未证明的风险、限制和下一步。

测试或 eval 尚未运行时，明确区分“场景已设计”“可以执行”“当前测不了”，不要编造实际表现。已经实际运行时，不能只给通过数、标签、文件链接或“见逐条评分”；每个重要结论都要让读者从正文看懂发生了什么。多次结果相同可以合并统计，但至少完整展开一个代表样本，并说明其余样本是否真的属于同一模式。

主文档不设置篇幅、表格列数、场景数或决策问题数上限。Source Ledger、Behavior/Risk/Fault 全量映射、原始轨迹、哈希和机器评分等复现材料可按需放进独立附件；任何用于理解结论的事实和解释必须留在主文档。
...
- 无关或只更换样例值的重复场景已合并，所有影响结论的场景仍在主文档中得到解释；
- 六个 Lens 都在主文档中明确标为 `已覆盖 / 部分覆盖 / 未覆盖 / 不适用`，且 `不适用` 有具体理由；
- 普通开发者只读主文档即可说明本次验证范围、各 Lens 的判断依据、重要场景的预期与实际、待决策项和下一步，不需要理解内部缩写或跨表追踪 ID；
