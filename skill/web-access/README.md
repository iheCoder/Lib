...
给 Codex 适配的完整联网能力 skill。

这个适配版不再假设 Claude Code 的 `WebSearch/WebFetch`，而是把 Codex 原生 `web` 能力和本机 CDP 浏览器能力统一起来。补上的核心是：**联网策略 + CDP 浏览器操作 + 站点经验积累**。
...
| 能力 | 说明 |
|------|------|
| 联网工具自动选择 | Codex `web` / curl / Jina / CDP，按场景自主判断，可任意组合 |
...
```bash
# 启动
node /Users/tal/GolandProjects/Lib/skill/web-access/scripts/cdp-proxy.mjs &
